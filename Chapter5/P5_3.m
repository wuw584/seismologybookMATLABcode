...
%P5_3.m
H=30; ?壳的厚度为30km
vs1=3.6;vs2=4.6;?壳和地幔的速度，单位km/s
miu21=1.8;?切模量的比值
c=vs1:0.01:vs2;??速度序列
c2=c.*c;??速度的平方
sqcl=sqrt(1/vsl/vsl-1/c2);
Bgc2=aqrt(1/c2-1/vs2/vs2); √是
atann=atan(miu21.*sqe2./sgel); *am 2√1/e2-1/:
g2√1/β?-1/c2
omiga0=c./(H*sqcl).*(atann);??据式(5-1-17)计算基阶频率
omiga1=c./(H*sqc1).*(atann+1*pi);??据式(5-1-17)计算一阶频率
omiga2=c./(H*sqc1).*(atann+2*pi);??据式(5-1-17)计算二阶频率
omiga3=c./(H*sqe1).*(atann+3*pi);??据式(5-1-17)计算三阶频率
figure(1)
semilogx(2*pi./omiga0,c,'-',2*pi./omigal,c,':',2*pi./omiga2,c,'--',2*pi./omiga3,c,'
-.')8以半对数轴绘制频率，相速度曲线
...
sqe2=sqrt(1/c2-1/vs2/vs2); √是-
atann=atan(miu21.*sqc2./sqcl); *aretn t2√1/c2-1/
√1/B?-1/c:
omiga0=c./(H*sqc1).*(atann+0*pi);&根据式(5-1-17)计算基阶频率
zl=[0:30]; ?壳深度范围
D0=cos((omiga0/c*sqe1).*z1);??据式(5-1-25)的第一式计算基阶振型地壳中相对振幅
z2=[31:40]; ?幔范围
D0=[D0,cos(omiga0/c*sqc1*H)*exp(-omiga0/c*sqe2*(z2-H))];*根据式(5-1-25)第二式计
算基阶振型地幔的相对振幅
omiga1=c./(H*sqc1).*(atann+1*pi);??据式(5-1-17)计算一阶频率
D1=cos((omigal/c*sqc1).*[0:30]);根据式(5-1-25)的第一式计算一阶振型地壳中相对振幅
D1=[D1,cos(omigal/c*sqcl*H)*exp(-omigal/c*sqe2*([31:40]-H))];??据根据式(5-1-25)
的第二式计算一阶振型地幔相对振幅
omiga2=c./(H*sqe1).*(atann+2*pi);??据式(5-1-17)计算二阶频率
D2=cos((omiga2/c*sqc1).*[0:30]);??据式(5-1-25)的第一式计算二阶振型地壳相对振幅
D2=[D2,cos(omiga2/c*sqcl*H)*exp(-omiga2/c*sqc2*([31:40]-H))];??据根据式(5-1-25)
的第二式计算二阶振型地幔相对振幅
fill([-1,1,1,-1,-1],[40,40,30,30,40],'y1);??地幔涂为黄色
hold on ?形保持
